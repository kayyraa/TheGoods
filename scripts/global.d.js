...
globalThis.FirebaseConfig = {
    apiKey: "AIzaSyDPI04MTFq8JpJtGQXbf1xUTWyqfuxotdg",
    authDomain: "thegoods-ec9ec.firebaseapp.com",
    projectId: "thegoods-ec9ec",
    storageBucket: "thegoods-ec9ec.firebasestorage.app",
    messagingSenderId: "427481380925",
    appId: "1:427481380925:web:7b5f94c1ccb25562651254"
};
...
globalThis.FramesContainer = document.querySelector(".Frames");
globalThis.Frames = Array.from(FramesContainer.children);
...
globalThis.TransactionHistory = document.querySelector(".TransactionHistory");
...
document.querySelectorAll(`[class*="Button"]`).forEach(Button => {
    let Name = Button.className.replace("Button", "").trim();
    if (Buttons[Name]) {
        if (Array.isArray(Buttons[Name])) {
            Buttons[Name].push(Button);
        } else {
            Buttons[Name] = [Buttons[Name], Button];
        }
    } else Buttons[Name] = Button;
});
...
globalThis.GithubStorage = class {
    ...
    static DownloadFromUrl(Path) {
        return `https://github.com/kayyraa/DirectStorage/blob/main/${Path}}?raw=true`
    }
}
...
globalThis.Format = class {
    ...
    ConvertToCurrencyReadable(Pattern) {
        const Patterns = {
            "USD": "$",
            "EUR": "€",
            "GBP": "£",
        ...
        const Amount = parseFloat(this.Value.split("-")[0]).toFixed(2);
        const Currency = Patterns[Pattern || this.Value.split("-")[1]];
...
globalThis.SwitchFrame = (TargetFrame) => {
    ...
    localStorage.setItem("LastFrame", TargetFrame);
};
