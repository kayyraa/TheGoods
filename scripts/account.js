...
Buttons.AccountButton.forEach(Node => {
    Node.addEventListener("click", async () => {
        ...
        SetProfileImageButton.addEventListener("click", async () => {
            ...
            const Path = `thegoods/${Uuid(8)}.${ProfileImageFile.name.split(".")[1]}`;
        ...
        Submit.addEventListener("click", async () => {
            ...
            const User = { Username, Password, WalletId: Uuid(4), Wallet: "0-USD", ProfileImage: "https://github.com/kayyraa/DirectStorage/blob/main/kumchat/eadd-2a1f.svg?raw=true" };
...
if (localStorage.getItem("User")) {
    const UserDocuments = await Storage.GetDocumentsByField("Username", User.Username);
    const UserDocument = UserDocuments[0];
    if (UserDocument) Buttons.AccountButton.forEach(Node => Node.innerHTML = `<img src="${UserDocument.ProfileImage}">`);
}